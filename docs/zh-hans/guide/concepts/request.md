...
| 类别 | 方法 | 描述 |
|------|------|------|
| **请求信息** | `uri()/uri_mut()/set_uri()` | URI 操作 |
| | `method()/method_mut()` | HTTP 方法操作 |
| | `version()/version_mut()` | HTTP 版本操作 |
| | `scheme()/scheme_mut()` | 协议方案操作 |
| | `remote_addr()/local_addr()` | 地址信息 |
| **请求头** | `headers()/headers_mut()` | 获取全部请求头 |
| | `header<T>()/try_header<T>()` | 获取并解析特定头 |
| | `add_header()` | 添加请求头 |
| | `content_type()/accept()` | 获取内容类型/接受类型 |
| **参数处理** | `params()/param<T>()` | 路径参数操作 |
| | `queries()/query<T>()` | 查询参数操作 |
| | `form<T>()/form_or_query<T>()` | 表单数据操作 |
| **请求体** | `body()/body_mut()` | 获取请求体 |
| | `replace_body()/take_body()` | 修改/提取请求体 |
| | `payload()/payload_with_max_size()` | 获取原始数据 |
| **文件处理** | `file()/files()/all_files()` | 获取上传文件 |
| | `first_file()` | 获取第一个文件 |
| **数据解析** | `extract<T>()` | 统一数据提取 |
| | `parse_json<T>()/parse_form<T>()` | 解析 JSON/表单 |
| | `parse_body<T>()` | 智能解析请求体 |
| | `parse_params<T>()/parse_queries<T>()` | 解析参数/查询 |
| **特殊功能** | `cookies()/cookie()` | Cookie 操作 (需 cookie feature) |
| | `extensions()/extensions_mut()` | 扩展数据存储 |
| | `set_secure_max_size()` | 设置安全大小限制 |