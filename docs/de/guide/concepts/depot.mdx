...
2. **Typinjektion**: Speichern und Abrufen von Werten basierend auf ihrem Typ mittels `insert_typed`/`get_typed`-Methoden.
...
## Setzen und Abrufen von Daten über `insert_typed` und `get_typed`

Manchmal gibt es Szenarien, in denen für einen Typ eine eindeutige Instanz existiert, ohne dass diese einem spezifischen `Schlüssel` zugeordnet werden muss. Sie können `insert_typed` verwenden, um Daten einzufügen, und dann `get_typed`, um sie abzurufen. Diese Methoden erfordern keine Angabe eines `Schlüssels`.
...
| **Typinjektion/-abruf** | `insert_typed<V>(value)` | Injiziert einen Wert nach Typ |
| | `get_typed<T>()` | Ruft eine Referenz auf den injizierten Wert ab |
| | `get_typed_mut<T>()` | Ruft eine mutable Referenz auf den injizierten Wert ab |
| | `contains_typed<T>()` | Prüft, ob ein bestimmter Typ enthalten ist |
| | `remove_typed<T>()` | Entfernt und gibt den injizierten Wert zurück |
...
| | `remove(key).is_some()` | Löscht das Schlüssel-Wert-Paar |
