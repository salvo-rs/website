...
:::tip
現在、Salvoがサポートする最低Rustバージョンは1.92です。`rustup update`を実行して、要件を満たすRustがインストールされていることを確認してください。
:::
...
おめでとうございます！あなたの最初のSalvoプログラムが完成しました。コマンドラインで`cargo run`を実行し、ブラウザで`http://127.0.0.1:8698`を開くだけです。
