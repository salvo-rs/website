...
{/* add tips for intall oapi with cargo */}
:::tip
This is tips for install Oapi in Salvo
```bash
cargo add salvo --features oapi
```
:::
...
`#[derive(ToSchema)]` also supports enum types. Here are various enum usage patterns:

_**Simple Enum**_ — When all variants are unit types, generates a string enum:
...
_**Complex Enum**_ — When variants contain data, uses `oneOf` by default:
...
enum PaymentMethod {
    ...
    Cash,
    /// Card payment
    Card { card_number: String, cvv: String },
    /// E-wallet
    Wallet(String),
}
```

_**Tagged Enums**_ — Supports `tag`, `tag` + `content`, `untagged` serde enum representations:
...
_**Using enum fields in structs**_ :
...
### Use Function Parameter Extractors Instead of Manual Extraction
...
If you want protected endpoints to appear in Swagger UI's **Authorize** dialog, you need to wire the OpenAPI metadata explicitly:

- Register a security scheme with `OpenApi::add_security_scheme(...)`.
- Attach a security requirement with `#[endpoint(security(...))]` or, if the route is protected by middleware, with router metadata such as `.oapi_security(...)`.
- Optionally pass Swagger UI OAuth client settings with `SwaggerUi::oauth(...)`.

_**Example Code**_

<Tabs>
<Tab label="main.rs">
```rust file="<root>/codes/oapi-oauth2-password/src/main.rs"
```
</Tab>
<Tab label="Cargo.toml">
```toml file="<root>/codes/oapi-oauth2-password/Cargo.toml"
```
</Tab>
</Tabs>

In this example, the `/api/profile` route is protected by `JwtAuth` middleware, while the OpenAPI security declaration is attached manually with `.oapi_security(...)`.

Salvo does not currently infer Swagger/OpenAPI security declarations from middleware automatically. Middleware and documentation metadata are separate today, so when authentication is added through middleware you still need to declare the matching OpenAPI security scheme and requirement yourself.
