...
fn main() {
    DB_POOL
        .set(build_pool(&DB_URL, 10).expect(&format!("Errore di connessione a {}", &DB_URL)))
        .ok();
}
...
SeaORM è un ORM asincrono e dinamico che offre un solido supporto per database relazionali, inclusi relazioni tra entità, sistema di migrazioni e un costruttore di query type-safe completo. Ideale per progetti medio-grandi che necessitano di un ORM completo.

## [Tokio ORM (Toasty)](https://github.com/tokio-rs/toasty)

Toasty è un ORM sviluppato dal team Tokio, attualmente in fase di sviluppo attivo, focalizzato sull'offrire una soluzione ORM perfettamente integrata nell'ecosistema Tokio. Potrebbe essere adatto a progetti che dipendono da Tokio e sono disposti ad adottare tecnologie emergenti.

## [SurrealDB Rust SDK](https://surrealdb.com/docs/sdk/rust)

L'SDK Rust di SurrealDB fornisce la connettività con questo database multi-modello, ideale per applicazioni che gestiscono dati a grafo, documenti e relazionali. Un'ottima scelta per progetti che richiedono modelli di dati flessibili.
