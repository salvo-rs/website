...
:::warning
`Json<T>` rend un document JSON complet. Dans Salvo 0.94 et versions suivantes, rendre `Json` remplace les octets de corps deja mis en tampon dans `Response` au lieu de les ajouter. Pour emettre plusieurs enregistrements JSON, serialisez chaque enregistrement vous-meme et ajoutez-le avec `write_body`, par exemple pour produire du NDJSON.
:::
...
| | `render_with_status()` | Définir le code d'état et rendre le contenu |
