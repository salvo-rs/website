...
:::warning
`Json<T>` 会渲染一个完整的 JSON 文档。Salvo 0.94 及之后版本中，渲染 `Json` 会替换 `Response` 中已经缓冲的响应体字节，而不是追加到后面。如果需要输出多条 JSON 记录，请自行序列化每条记录并用 `write_body` 追加，例如生成 NDJSON。
:::
