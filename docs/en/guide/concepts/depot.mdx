...
2. **Type Injection**: Store and retrieve values based on type via `insert_typed`/`get_typed` methods.
...
## Setting and Retrieving Data via `insert_typed` and `get_typed`

Sometimes, there are scenarios where a unique instance exists for a type without needing to relate to a specific `key`. You can use `insert_typed` to insert data and then use `get_typed` to retrieve it. These methods do not require you to provide a `key`.
...
| **Type Injection/Retrieval** | `insert_typed<V>(value)` | Injects a value by type |
| | `get_typed<T>()` | Retrieves a reference to the injected value |
| | `get_typed_mut<T>()` | Retrieves a mutable reference to the injected value |
| | `contains_typed<T>()` | Checks if a specific type is contained |
| | `remove_typed<T>()` | Removes and returns the injected value |
...
| | `remove(key).is_some()` | Deletes the key-value pair |