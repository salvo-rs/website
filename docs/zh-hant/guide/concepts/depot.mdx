...
2. **型別注入**：透過 `insert_typed`/`get_typed` 方法基於型別存取值
...
## 透過 `insert_typed` 與 `get_typed` 設定及取出資料

有時存在某些不需關聯具體 `key`，且該型別僅需唯一實例的情況。此時可使用 `insert_typed` 插入資料，並以 `get_typed` 取出資料。此方法無需提供 `key`。
...
| **型別注入/取得** | `insert_typed<V>(value)` | 依型別注入值 |
| | `get_typed<T>()` | 取得注入值的參考 |
| | `get_typed_mut<T>()` | 取得注入值的可變參考 |
| | `contains_typed<T>()` | 檢查是否包含特定型別 |
| | `remove_typed<T>()` | 移除並回傳注入值 |
| **鍵值操作** | `insert(key, value)` | 插入鍵值對 |
| | `get<V>(key)` | 取得鍵對應值的參考 |
| | `get_mut<V>(key)` | 取得鍵對應值的可變參考 |
| | `contains_key(key)` | 檢查是否包含特定鍵 |
| | `remove<V>(key)` | 移除鍵並回傳值 |
| | `remove(key).is_some()` | 刪除鍵值對 |
