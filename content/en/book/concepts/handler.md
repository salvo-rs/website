...
The middleware is added through the ```before``` and ```after``` functions of the ``Router```. The added middleware will affect the current ```Router``` and its internal All descendants of ```Router```.
