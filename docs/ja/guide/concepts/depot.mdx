...
2. **型ベース注入**：`insert_typed`/`get_typed` メソッドを使用して、型に基づいて値を格納・取得します。
...
## `insert_typed` と `get_typed` によるデータの設定と取得

場合によっては、具体的な `key` に関係なく、その型に対して単一のインスタンスのみが存在するデータがあります。このような場合、`insert_typed` を使用してデータを挿入し、`get_typed` を使用してデータを取得できます。これらを使用する際に `key` を指定する必要はありません。
...
| **型注入/取得** | `insert_typed<V>(value)` | 型に基づいて値を注入 |
| | `get_typed<T>()` | 注入された値への参照を取得 |
| | `get_typed_mut<T>()` | 注入された値への可変参照を取得 |
| | `contains_typed<T>()` | 特定の型が含まれているか確認 |
| | `remove_typed<T>()` | 注入された値を削除して返却 |
