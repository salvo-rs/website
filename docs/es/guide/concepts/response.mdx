...
:::warning
`Json<T>` renderiza un documento JSON completo. En Salvo 0.94 y versiones posteriores, renderizar `Json` reemplaza los bytes del cuerpo ya almacenados en `Response` en lugar de anexarlos. Para emitir varios registros JSON, serializa cada registro manualmente y añádelo con `write_body`, por ejemplo al producir NDJSON.
:::
...
| | `render_with_status()` | Establecer código de estado y renderizar contenido |
