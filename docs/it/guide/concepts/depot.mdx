...
[Depot](https://docs.rs/salvo_core/latest/salvo_core/struct.Depot.html) viene utilizzato per memorizzare dati temporanei coinvolti in una singola richiesta. Il middleware può inserire i dati temporanei che elabora nel Depot per l'utilizzo da parte dei programmi successivi.
...
2. **Iniezione di Tipo**: Memorizza e recupera valori basati sul tipo tramite i metodi `insert_typed`/`get_typed`.
...
## Impostazione e Recupero Dati tramite `insert_typed` e `get_typed`

A volte, ci sono scenari in cui esiste un'istanza unica per un tipo senza bisogno di relazionarsi a una specifica `chiave`. Puoi usare `insert_typed` per inserire dati e poi usare `get_typed` per recuperarli. Questi metodi non richiedono di fornire una `chiave`.

```rust
depot.insert_typed(Config::new());
depot.get_typed::<Config>();
```
...
| **Iniezione/Recupero di Tipo** | `insert_typed<V>(valore)` | Inietta un valore per tipo |
| | `get_typed<T>()` | Recupera un riferimento al valore iniettato |
| | `get_typed_mut<T>()` | Recupera un riferimento mutabile al valore iniettato |
| | `contains_typed<T>()` | Verifica se è contenuto un tipo specifico |
| | `remove_typed<T>()` | Rimuove e restituisce il valore iniettato |
...
| | `remove(chiave).is_some()` | Elimina la coppia chiave-valore |
