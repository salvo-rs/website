...
2. **Injeção de Tipo**: Armazena e recupera valores com base no tipo através dos métodos `insert_typed`/`get_typed`.
...
## Configurando e Recuperando Dados via `insert_typed` e `get_typed`

Às vezes, há cenários em que existe uma instância única para um tipo sem necessidade de relacioná-la a uma `chave` específica. Você pode usar `insert_typed` para inserir dados e, em seguida, usar `get_typed` para recuperá-los. Esses métodos não exigem que você forneça uma `chave`.
...
| **Injeção/Recuperação por Tipo** | `insert_typed<V>(value)` | Injeta um valor por tipo |
| | `get_typed<T>()` | Recupera uma referência ao valor injetado |
| | `get_typed_mut<T>()` | Recupera uma referência mutável ao valor injetado |
| | `contains_typed<T>()` | Verifica se um tipo específico está contido |
| | `remove_typed<T>()` | Remove e retorna o valor injetado |
...
| | `remove(key).is_some()` | Exclui o par chave-valor |
