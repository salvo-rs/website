...
`#[derive(ToSchema)]` 同样支持枚举类型。以下是各种枚举的使用方式：

_**简单枚举（Simple Enum）**_ — 所有变体都是 Unit 类型时，生成字符串枚举：
...
_**带数据的枚举（Complex Enum）**_ — 变体包含数据时，默认使用 `oneOf`：
...
_**使用 serde 标签的枚举**_ — 支持 `tag`、`tag` + `content`、`untagged` 等 serde 枚举表示：
...
**重要**: 为了让 OpenAPI 文档能够自动生成，请使用函数参数提取器，而不是手动从 `Request` 中提取数据。

```rust
// ❌ 错误做法: 手动提取 - 不会生成 OpenAPI 文档
#[endpoint]
async fn get_user(req: &mut Request) -> Json<User> {
    let id = req.param::<i64>("id").unwrap();  // 不会被文档化!
    let page = req.query::<i32>("page");        // 不会被文档化!
    // ...
}

// ✅ 正确做法: 使用参数提取器 - 自动生成 OpenAPI 文档
#[endpoint]
async fn get_user(
    id: PathParam<i64>,           // 自动文档化为路径参数
    page: QueryParam<i32, false>, // 自动文档化为可选查询参数
) -> Json<User> {
    // ...
}
```

当您使用 `PathParam`、`QueryParam`、`JsonBody` 等提取器时，OpenAPI 生成器可以：
- 自动检测参数名称和类型
- 标记参数是必需的还是可选的
- 生成正确的 schema 定义

### 返回 Result 类型以获取完整的响应文档

**重要**: 返回 `Result<T, E>`，其中 `T` 和 `E` 都实现了 `Writer` 和 `EndpointOutRegister`。这样可以自动生成成功响应和错误响应的文档。
...
| 模式 | 参数是否被文档化 | 响应是否被文档化 |
|------|-----------------|-----------------|
| `req.param()` / `req.query()` | ❌ 否 | - |
| `PathParam<T>` / `QueryParam<T>` | ✅ 是 | - |
| `-> Json<T>` | - | ✅ 仅成功响应 |
| `-> Result<Json<T>, StatusError>` | - | ✅ 成功 + 错误 |
| `-> Result<Json<T>, CustomError>` | - | ✅ 完全控制 |

## Swagger UI 鉴权

如果你希望受保护的接口出现在 Swagger UI 的 **Authorize** 弹窗里，需要显式把 OpenAPI 元数据接上：

- 用 `OpenApi::add_security_scheme(...)` 注册安全方案。
- 用 `#[endpoint(security(...))]` 给接口声明安全需求；如果鉴权是通过中间件挂到路由上的，则使用 `.oapi_security(...)` 这类路由元数据来补上声明。
- 如果 Swagger UI 还需要 OAuth 客户端侧配置，可以再通过 `SwaggerUi::oauth(...)` 传入。
...
这个示例里，`/api/profile` 由 `JwtAuth` 中间件保护，而 OpenAPI 的安全声明则通过 `.oapi_security(...)` 手工附加到路由上。

Salvo 目前还不能根据 middleware 自动推导 Swagger/OpenAPI 的 security declaration。也就是说，鉴权中间件和文档元数据现在仍然是两套信息；如果鉴权是通过 middleware 接入的，就需要你手工补上对应的 OpenAPI security scheme 和 security requirement。
