...
:::warning
`Json<T>` は完全な JSON ドキュメントを 1 つ描画します。Salvo 0.94 以降では、`Json` を描画すると `Response` に既にバッファされたボディバイトを追記せず置き換えます。複数の JSON レコードを出力する場合は、各レコードを自分でシリアライズし、NDJSON などとして `write_body` で追記してください。
:::
...
| | `render_with_status()` | ステータスコードを設定し、コンテンツをレンダリング |
| | `send_file()` | ファイルを送信 |
