...
SeaORMは非同期で動的なORMで、エンティティ関係、マイグレーションシステム、完全な型安全なクエリビルダーなど、強力なリレーショナルデータベースサポートを提供します。フル機能のORMが必要な中規模から大規模プロジェクトに適しています。

## [Tokio ORM (Toasty)](https://github.com/tokio-rs/toasty)

ToastyはTokioチームが開発しているORMで、現在も活発に開発中です。Tokioエコシステムとの緊密な統合を目指しており、Tokioに依存し、新興技術を採用することに抵抗のないプロジェクトに適している可能性があります。

## [SurrealDB Rust SDK](https://surrealdb.com/docs/sdk/rust)

SurrealDBのRust SDKは、このマルチモデルデータベースとの接続機能を提供します。グラフデータ、ドキュメントデータ、リレーショナルデータを扱う必要があるアプリケーションに適しています。柔軟なデータモデルが必要なプロジェクトに良い選択肢となるでしょう。
