...
2. **类型注入**：通过 `insert_typed`/`get_typed` 方法基于类型存取值
...
## 通过 `insert_typed` 和 `get_typed` 设置和取出数据

有时, 存在一些不需要关系具体 `key`, 对于这种类型也存在唯一实例的情况. 可以使用 `insert_typed` 插入数据, 然后使用 `get_typed` 取出数据. 它们不需要你提供 `key`.
...
| **类型注入/获取** | `insert_typed<V>(value)` | 按类型注入值 |
| | `get_typed<T>()` | 获取注入值引用 |
| | `get_typed_mut<T>()` | 获取注入值可变引用 |
| | `contains_typed<T>()` | 检查是否包含特定类型 |
| | `remove_typed<T>()` | 移除并返回注入值 |
| **键值操作** | `insert(key, value)` | 插入键值对 |
| | `get<V>(key)` | 获取键对应值引用 |
| | `get_mut<V>(key)` | 获取键对应值可变引用 |
| | `contains_key(key)` | 检查是否包含特定键 |
| | `remove<V>(key)` | 移除键并返回值 |
| | `remove(key).is_some()` | 删除键值对 |
