...
Para achatar uma sub-estrutura extraível, use `#[salvo(extract(flatten))]`. `#[serde(flatten)]` não é suportado em campos de `#[derive(Extractible)]` e deve ser substituído por `#[salvo(extract(flatten))]`.
