...
:::warning
`Json<T>` renderiza um documento JSON completo. No Salvo 0.94 e versoes posteriores, renderizar `Json` substitui os bytes do corpo ja armazenados em `Response` em vez de anexa-los. Para emitir varios registros JSON, serialize cada registro manualmente e anexe-o com `write_body`, por exemplo ao produzir NDJSON.
:::
...
| | `render_with_status()` | Definir código de status e renderizar conteúdo |
