...
2. **Injection de type** : Stocker et récupérer des valeurs basées sur le type via les méthodes `insert_typed`/`get_typed`.
...
## Définition et récupération de données via `insert_typed` et `get_typed`

Parfois, il existe des scénarios où une instance unique existe pour un type sans avoir besoin de la relier à une `clé` spécifique. Vous pouvez utiliser `insert_typed` pour insérer des données, puis utiliser `get_typed` pour les récupérer. Ces méthodes ne nécessitent pas que vous fournissiez une `clé`.
...
| **Injection/Récupération par type** | `insert_typed<V>(value)` | Injecte une valeur par type |
| | `get_typed<T>()` | Récupère une référence à la valeur injectée |
| | `get_typed_mut<T>()` | Récupère une référence mutable à la valeur injectée |
| | `contains_typed<T>()` | Vérifie si un type spécifique est contenu |
| | `remove_typed<T>()` | Supprime et retourne la valeur injectée |
...
| | `remove(key).is_some()` | Supprime la paire clé-valeur |
