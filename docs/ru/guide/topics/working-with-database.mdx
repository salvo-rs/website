...
SeaORM — это асинхронный динамический ORM, предоставляющий надёжную поддержку реляционных баз данных, включая связи между сущностями, систему миграций и полнофункциональный типобезопасный построитель запросов. Он хорошо подходит для средних и крупных проектов, требующих полноценного ORM.

## [Tokio ORM (Toasty)](https://github.com/tokio-rs/toasty)

Toasty — это ORM, разрабатываемый командой Tokio, который в настоящее время находится в активной разработке. Он ориентирован на предоставление тесно интегрированного ORM-решения для экосистемы Tokio. Может быть подходящим выбором для проектов, использующих Tokio и готовых к внедрению новых технологий.

## [SurrealDB Rust SDK](https://surrealdb.com/docs/sdk/rust)

SurrealDB Rust SDK обеспечивает подключение к этой многомодельной базе данных, что делает его подходящим для приложений, работающих с графовыми, документными и реляционными данными. Это отличный выбор для проектов, требующих гибких моделей данных.
