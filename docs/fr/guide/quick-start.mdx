...
:::tip
La version minimale de Rust prise en charge par Salvo est la 1.85. Exécutez `rustup update` pour vous assurer d'avoir une version compatible de Rust installée.
:::
...
Félicitations ! Votre premier programme Salvo est terminé. Exécutez simplement `cargo run` dans la ligne de commande, puis ouvrez `http://127.0.0.1:5800` dans votre navigateur.
