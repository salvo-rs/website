...
:::warning
`Json<T>` rendert ein vollstaendiges JSON-Dokument. In Salvo 0.94 und spaeter ersetzt das Rendern von `Json` bereits in `Response` gepufferte Body-Bytes, statt sie anzuhängen. Um mehrere JSON-Datensaetze auszugeben, serialisieren Sie jeden Datensatz selbst und haengen ihn mit `write_body` an, zum Beispiel fuer NDJSON.
:::
...
| | `render_with_status()` | Statuscode setzen und Inhalt darstellen |
