...
:::warning
`Json<T>` renderizza un documento JSON completo. In Salvo 0.94 e versioni successive, renderizzare `Json` sostituisce i byte del corpo gia presenti in `Response` invece di accodarli. Per emettere piu record JSON, serializza manualmente ogni record e accodalo con `write_body`, ad esempio producendo NDJSON.
:::
...
| | `render_with_status()` | Imposta il codice di stato e renderizza il contenuto |
