...
根據匹配階段收集到的 `Handler` 列表依次執行 `Handler`。執行過程中前面的中介軟體可以呼叫 `ctrl::call_next()` 讓後續的中介軟體先執行，然後再執行自己的邏輯。如果執行過程中出現狀態碼錯誤或轉向，那麼後續的 `Handler` 將不再執行，此時如果狀態碼是錯誤，並且 `Response` 的 `Body` 未設定或是 `ResBody::Error`，則進入錯誤捕獲階段，反之跳過捕獲階段。
...
`Catcher` 預設必須包含一個 `Handler` 用於作為預設錯誤處理。預設的是 `DefaultGoal`，你也可以完全自訂自己的 `Handler` 作為錯誤處理的預設實現。它會根據請求頭要求的 `content-type` 顯示對應格式的錯誤資訊，支援 `json`、`xml`、`text`、`html` 四種顯示格式。`DefaultGoal` 也提供了一些顯示設定，例如，預設它在顯示 html 格式時會顯示 salvo 相關連結，就可以呼叫 `DefaultGoal::footer` 或 `DefaultGoal::with_footer` 把頁腳設定成你自己希望的自訂頁腳。
