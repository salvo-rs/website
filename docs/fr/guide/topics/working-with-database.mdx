...
SeaORM est un ORM asynchrone et dynamique offrant une prise en charge robuste des bases de données relationnelles, incluant les relations d'entités, un système de migrations et un constructeur de requêtes type-safe complet. Idéal pour les projets de moyenne à grande envergure nécessitant un ORM complet.

## [Tokio ORM (Toasty)](https://github.com/tokio-rs/toasty)

Toasty est un ORM développé par l'équipe Tokio, actuellement en développement actif, qui se concentre sur la fourniture d'une solution ORM étroitement intégrée à l'écosystème Tokio. Peut convenir aux projets dépendant de Tokio et ouverts à l'adoption de technologies émergentes.

## [SurrealDB Rust SDK](https://surrealdb.com/docs/sdk/rust)

Le SDK Rust de SurrealDB offre des capacités de connexion à cette base de données multi-modèles, adaptée aux applications nécessitant de traiter des données de graphe, des documents et des données relationnelles. Un choix judicieux pour les projets nécessitant des modèles de données flexibles.
