...
async fn main() {
    ...
    // informazioni connessione mysql
...
SeaORM è un ORM asincrono e dinamico che offre un solido supporto per database relazionali, includendo relazioni tra entità, sistema di migrazioni e un costruttore di query type-safe completo. Ideale per progetti medio-grandi che richiedono un ORM completo.

## [Tokio ORM (Toasty)](https://github.com/tokio-rs/toasty)

Toasty è un ORM sviluppato dal team di Tokio, attualmente in fase di sviluppo attivo, focalizzato sull'offrire una soluzione ORM strettamente integrata nell'ecosistema Tokio. Potrebbe essere adatto a progetti che dipendono da Tokio e sono disposti ad adottare tecnologie emergenti.

## [SurrealDB Rust SDK](https://surrealdb.com/docs/sdk/rust)

L'SDK Rust di SurrealDB fornisce connettività con questo database multi-modello, adatto per applicazioni che necessitano di gestire dati grafici, documentali e relazionali. Un'ottima scelta per progetti che richiedono modelli di dati flessibili.
