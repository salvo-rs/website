...
此外，如果结构体实现了相关的trait，Salvo可以自动生成OpenAPI文档以及提取参数，自动处理不同的错误，并返回友好提示。这使得编写handler就像编写普通函数一样简单直观。在后续教程中，我们会逐步详细讲解这些功能，下面是一个例子：
...
在这个例子中，`JsonBody<CreateOrUpdateMessageLog>` 会自动从请求体中解析JSON数据并转换为 `CreateOrUpdateMessageLog` 类型（也支持多数据源，嵌套类型），同时 `#[endpoint]` 宏会自动为此接口生成OpenAPI文档，并简化了参数提取和错误处理的代码。
