...
### Diagramma di Flusso dell'Handler in Salvo
...
flowchart TD
    ...
    subgraph "Implementazione Handler"
        M1[Middleware di Logging] -->|Elaborazione Verticale| M2[Middleware di Autenticazione] -->|Elaborazione Verticale| M3[Altri Middleware di Business]
...
L'essenza del modello a cipolla è che posizionando `ctrl.call_next()` prima e dopo una logica specifica, si implementa un flusso di elaborazione bidirezionale per richieste e risposte, consentendo a ogni middleware di partecipare al ciclo completo richiesta-risposta.

### Struttura di Esempio Completa di un Middleware
...
flowchart TB
    ...
    subgraph "Flusso di Elaborazione del Modello a Cipolla"
        direction TB
        ...
        subgraph M2["Middleware 2"]
            M2A["Pre-elaborazione<br>Registra Ora Inizio Richiesta"] --> M2B["Post-elaborazione<br>Calcola Tempo di Elaborazione"]
        end
...
Un `Handler` può anche essere utilizzato come un `Handler` che partecipa alla corrispondenza delle rotte e viene eseguito alla fine, noto come `goal`.
...
La macro `#[handler]` può semplificare notevolmente la scrittura del codice e migliorarne la flessibilità.
...
`#[handler]` può essere applicato non solo alle funzioni, ma anche al blocco `impl` di una `struct` per far sì che la `struct` implementi `Handler`. In questo caso, la funzione `handle` all'interno del blocco `impl` viene riconosciuta come l'implementazione concreta del metodo `handle` in `Handler`:
...
Per i tipi di errore personalizzati, è possibile visualizzare diverse pagine di errore secondo necessità.
...
```
{/* Auto generato, hash file originale:aba76a058ee7c27a6a982876209eae78 */}
{/* Auto generated, origin file hash:a54688cec12a2532c72e4e8cdb39a3aa */}